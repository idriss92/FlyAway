...
namespace FlyAway.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Manage(ManageUserViewModel model)
        {
            ...
            {
                // L'utilisateur n'ayant pas de mot de passe, supprimez toute erreur de validation provoquée par un champ OldPassword manquant
        ...
        {
            // Demander une redirection vers le fournisseur de connexion externe  pour lier une connexion pour l'utilisateur actuel
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    if (result.Succeeded)
                    {
                        ...
                        // string code = await UserManager.GenerateEmailConfirmationTokenAsync(user.Id);
                        // var callbackUrl = Url.Action("ConfirmEmail", "Account", new { userId = user.Id, code = code }, protocol: Request.Url.Scheme);
                        // SendEmail(user.Email, callbackUrl, "Confirmer votre compte", "Veuillez confirmer votre compte en cliquant sur ce lien");
        ...
        {
            // Pour plus d'informations sur l'envoi de courrier, consultez http://go.microsoft.com/fwlink/?LinkID=320771
        }
